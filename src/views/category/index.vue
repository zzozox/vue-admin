...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      this.axios({
        method: 'GET',
        url: 'http://localhost:8083/milimili/admin/searchVideoType?pageNum=' + vm.listQuery.page + '&pageSize=' + vm.listQuery.limit + '&typeName=' + vm.videoTypeName
      }).then(function(resp) {
    ...
    getList() {
      ...
      this.axios({
        method: 'GET',
        url: 'http://localhost:8083/milimili/admin/categoryPageInfo?pageNum=' + vm.listQuery.page + '&pageSize=' + vm.listQuery.limit
      }).then(function(resp) {
        vm.total = resp.data.total // 讲pageInfo中的total放到vm的total
        vm.list = resp.data.list
      })
    ...
    delCategory(id) {
      ...
      this.$confirm('此操作将删除分类, 是否继续?', '提示', {
        ...
        vm.axios({
          method: 'GET',
          url: 'http://localhost:8083/milimili/admin/deleteCategory?id=' + id
        }).then(function(resp) {
